...
class Device(
    ...
    fun connect(
        ...
        if(bluetoothGatt != null){
          bluetoothGatt?.close();
          val isCacheRefreshed = refreshDeviceCache(bluetoothGatt);
          Logger.debug(if (isCacheRefreshed == true)  "Cache refreshed successfully." else "Failed to refresh cache.")
        }
        disconnect(timeout) {};
        connectionState = STATE_CONNECTING
        // ccastro
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
          Logger.debug("Common flow v1 ${Build.VERSION.SDK_INT}")
          ...
          Logger.debug("Common flow v2 ${Build.VERSION.SDK_INT}")
          bluetoothGatt = device.connectGatt(
              context, autoConnect, gattCallback, BluetoothDevice.TRANSPORT_AUTO
          )
        } else {
          Logger.debug("Default flow ${Build.VERSION.SDK_INT}")
          bluetoothGatt = device.connectGatt(
  ...
  fun restartBluetooth() {
    ...
    Handler().postDelayed({ bluetoothAdapter.enable() }, 1000)
  }
